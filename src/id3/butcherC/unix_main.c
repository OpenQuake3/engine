...
void tty_Show(void) {
  ...
  if (ttycon_hide > 0) {
    ...
    if (ttycon_hide == 0) {
      ...
      if (tty_con.cursor > 0) { write(STDOUT_FILENO, tty_con.buffer, tty_con.cursor); }
    }
  }
}
...
void floating_point_exception_handler(int whatever) { signal(SIGFPE, floating_point_exception_handler); }
...
tty_err Sys_ConsoleInputInit(void) {
  ...
  if (signal(SIGTSTP, SIG_IGN) == SIG_DFL) { signal(SIGTSTP, CON_SigTStp); }
  ...
  */
  ...
  if (ttycon_ansicolor && ttycon_ansicolor->integer) { ttycon_color_on = qtrue; }
...
static const char* getANSIcolor(char Q3color) {
  ...
  for (i = 0; i < ARRAY_LEN(tty_colorTable); i++) {
    if (Q3color == tty_colorTable[i].Q3color) { return tty_colorTable[i].ANSIcolor; }
  }
  return NULL;
}
...
void QDECL Sys_SetStatus(const char* format, ...) { return; }
...
static void Sys_SetBinaryPath(const char* path) {
  ...
  if (d != NULL && d != binaryPath) { Q_strncpyz(binaryPath, d, sizeof(binaryPath)); }
}
...
=================
*/
static void Sys_SetDefaultBasePath(const char* path) { Q_strncpyz(installPath, path, sizeof(installPath)); }
...
static char* Sys_StripAppBundle(char* dir) {
  ...
  if (strcmp(basename(cwd), "MacOS") != 0) { return dir; }

  Q_strncpyz(cwd, dirname(cwd), sizeof(cwd));
  if (strcmp(basename(cwd), "Contents") != 0) { return dir; }

  Q_strncpyz(cwd, dirname(cwd), sizeof(cwd));
  if (strstr(basename(cwd), ".app") == NULL) { return dir; }
...
#ifdef __APPLE__
  if (installPath[0] != '\0') return installPath;
  else
#endif
    return Sys_Pwd();
}
...
const char* Sys_BinName(const char* arg0) {
  ...
  if (n >= 0 && n < PATH_MAX) dst[n] = '\0';
  else Q_strncpyz(dst, arg0, PATH_MAX);
#elif defined(__APPLE__)
  uint32_t bufsize = sizeof(dst);

  if (_NSGetExecutablePath(dst, &bufsize) == -1) { Q_strncpyz(dst, arg0, PATH_MAX); }
#else
...
/* #endif */
  ...
  for (len = 1, i = 1; i < argc; i++) len += strlen(argv[i]) + 1;
