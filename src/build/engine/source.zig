...
const dirs = struct {
  const client = &[_]confy.Glob{
    ...
    .{.dir= Engine.dir.src++"/qcommon",        .opts= .{.filter= .{.excludes= &.{ "vm_x86", "vm_aarch64", "vm_armv7l", "vm_powerpc" }}}},
  ...
  const server = &[_]confy.Glob{
    .{.dir= Engine.dir.src++"/botlib" },
    .{.dir= Engine.dir.src++"/qcommon",  .opts= .{.filter= .{.excludes= &.{ "vm_x86", "vm_aarch64", "vm_armv7l", "vm_powerpc", "puff" }}}},
    .{.dir= Engine.dir.src++"/server",   .opts= .{.filter= .{.excludes= &.{ "sv_rankings" }}}},
  };
  //__________________
...
const files = struct {
  ...
  const snd_mix_x86_64 = &[_]confy.cstring{ Engine.dir.src++"/asm/snd_mix_x86_64.S" };
};
...
pub const client = struct {
  ...
  pub fn files (system :confy.System) confy.cstring_List { return switch (system.cpu) {
    .x86_64  => Engine.code.files.vm_x86 ++ Engine.code.files.snd_mix_x86_64,
